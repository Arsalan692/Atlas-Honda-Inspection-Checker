...
class DataHolder(BaseModel):
    model: Literal["CD70", "CD150", "CD125"] = Field(description="Stores the model of honda bike")
    frame_number: str = Field(description="Frame number of bike")
    defected_part_names: Annotated[list[str], ..., "List of defected parts name"]
    defect_observed_section: Annotated[list[str], ..., "List of problems which are under the 'DEFECT OBSERVED & REWORK CONFIRMATION' section"]


structured_llm = llm.with_structured_output(DataHolder)
    ...
    if analyze_button and final_image:
        with st.spinner("⚙️ Processing image..."):
            ...
            message = HumanMessage(content=[
                {"type": "text", "text": """
                    You are an OCR checker for a structured inspection sheet.

                    Extract the following:
                    1. Bike model (top-left corner of page).
                    2. Frame number (under the 'FRAME NO.' section).
                    3. Defected parts: Look ONLY at the 'CHECK' column.
                    4. Problems under the 'DEFECT OBSERVED & REWORK CONFIRMATION' section at the bottom.

                    Rules for extracting defected parts:
                    - A part is considered defected ONLY if there is a **clear, dark, complete tick (✓)** in the SAME row.
                    - Only take the ITEM NAME written in that row (do not take text from above or below).
                    - Ignore all rows without a tick.
                    - Ignore all notes, comments, or side markings (like dates, initials, handwriting).
                    - If a tick is in the box, return ONLY the item name in that row as it appears under "ITEMS".

                    Rules for extracting 'DEFECT OBSERVED & REWORK CONFIRMATION':
                    - Look at the handwritten section near the bottom.
                    - Capture each problem/issue written there as a separate string in a list.
                    - Do not summarize or rephrase, keep the wording as-is.

                    Return results in **JSON format matching this schema**:
                    {
                        "model": "...",
                        "frame_number": "...",
                        "defected_part_names": ["...", "..."],
                        "defect_observed_section": ["...", "..."]
                    }

                    If no parts are ticked, return an empty list [] for defected_part_names.
                    If no handwritten defects are found, return an empty list [] for defect_observed_section.
                """},
        ...
        if len(response.defected_part_names) == 0:
            if len(response.defect_observed_section) == 0:
                st.markdown(
                    f"""
                    <div style="
                        background: rgba(20, 20, 20, 0.85);
                        ...
                        text-align: center;
                    ">
                        <h3 style="color:#FFD700; margin-bottom:15px;">✅ Analysis Result</h3>
                        <p style="color:#FFD700; font-size:1.2rem; margin:0;">
                            <b>Model:</b> {response.model} <br>
                            <b>Frame No:</b> {response.frame_number}
                        </p>
                        <p style="color:#FF4444; font-size:1.2rem; margin:0;">🚫 No tick marks found</p>
                    </div>
                ...
                problems_names = response.defect_observed_section
                st.markdown(
                    f"""
                    <div style="
                        background: rgba(20, 20, 20, 0.85);
                    ...
                    ">
                        ...
                        <ul style="color:#E0E0E0; font-size:1.1rem; line-height:1.6;">
                            {''.join([f"<li style='margin-bottom:6px;'>⚠️ {p}</li>" for p in problems_names])}
                        </ul>
                    </div>
            ...
            parts = response.defected_part_names
            problems_names = response.defect_observed_section
            st.markdown(
                f"""
                <div style="
                ...
                ">
                    ...
                    <p style="color:#FFD700; font-size:1.2rem; margin-bottom:12px;">
                        <b>Model:</b> {response.model} <br>
                        <b>Frame No:</b> {response.frame_number}
                    </p>
                    <h5 style="color:white; text-align:left;">
                    ...
                    <ul style="color:#E0E0E0; font-size:1.1rem; line-height:1.6;">
                        {''.join([f"<li style='margin-bottom:6px;'>🔧 {p}</li>" for p in parts])}
                    </ul>
                    <h5 style="color:white; text-align:left;">
                        Observed Problems:
                    </h5>
